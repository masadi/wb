<template>
  <div>
    ...
    <b-modal size="lg" title="Detil Laporan Verifikasi" v-model="showModal">
      <label>Adakah pengisian APM yang tidak sesuai setelah diverifikasi?</label>
      <p>{{modalText.pengisian}}</p>
      <label>Adakah Kendala dalam Proses Verifikasi? dan cara mengatasainya?</label>
      <p>{{modalText.kendala}}</p>
      <label>Bagaimana Kondisi dari Tim Pendamping pada saat melaksankanan Workshop Pertama?</label>
      <p>{{modalText.kondisi}}</p>
    </b-modal>
  </div>
</template>
